    ...
    try{
        ...
        if( tokenRole !== "seller"){
            return next(new HttpError('You are not a seller', 403))
                ...
                console.error("Add Book Error:", error);  // 👈 this will help debug
...
export const editBook = async (req, res, next) =>{
    try{
        ...
        //   const existingBook = await Book.findOne({_id:id,seller:sellerId,is_deleted: false})
        //   if(!existingBook){
...
//view single book of logged in seller

// export const viewSellerBook = async (req,res,next) => {
//     try{

//         const {user_id : sellerId, user_role : tokenRole} =  req.user_data
//         if( tokenRole !== "seller"){
//             return next(new HttpError("Access denied. Only sellers are allowed.",401))
//         }
//         else{
...
//                 const viewBook = await Book.findOne({_id:id, seller:sellerId,is_deleted:false})
...
export const viewBook = async (req, res, next) => {
    try{
        ...
        if(!mongoose.Types.ObjectId.isValid(id)){
                return next(new HttpError("Id is not valid",400))
            }
...
// // List books of the logged-in seller
// export const listSellerBooks = async (req,res,next) => {
//     try{
//         //data from middleware
//         const {user_id: sellerId, user_role: tokenRole} = req.user_data
...
//                 status: true,
//                 data: bookList,
//                 message: 'Here is the list of your book'
//             })
